...
def fit_rubin_roman(event_params, path_save, path_ephemerides, algo, wfirst_lc, lsst_u, lsst_g, lsst_r, lsst_i, lsst_z, lsst_y):
    ...
    t0,u0,tE,s,q,alpha,piEN,piEE = float(event_params['t0']),float(event_params['u0']),float(event_params['te']),np.log10(float(event_params['s'])),np.log10(float(event_params['q'])), float(event_params['alpha']),float(event_params['piEN']), float(event_params['piEE'])
